...
1. `grep -i "pattern" <file>` searches through the desired files for the pattern without any case sensitivity and returns the lines where the pattern appears.
...
3. `grep -v "pattern" <file>` searches through the desired files for lines that *do not* contain the pattern.
...
see before and after the pattern we are searching for. Note: the line containing the pattern itself is also returned.
